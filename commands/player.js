...
module.exports = class extends Command {
  ...
  async run(msg, lang, args) {
    ...
    embed.setTitle(`${util.stripColor(player.prefix)} ${player.displayname}`)
    ...
    embed.addField('Rank', `${player.rank} (${util.convertRank(player.packageRank)})`)
    embed.addField('Level', `${util.getLevel(player.networkExp)} (Exact: ${util.getExactLevel(player.networkExp)})`, true)
    embed.addField('Experience', player.networkExp.toLocaleString(), true)
    embed.addField('Karma', player.karma.toLocaleString())
